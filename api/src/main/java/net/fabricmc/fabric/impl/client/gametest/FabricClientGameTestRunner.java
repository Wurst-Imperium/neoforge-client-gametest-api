...
public class FabricClientGameTestRunner
{
	...
	public static ClientGameTestEntrypoint currentlyRunningGameTest = null;
	...
	public static void start()
	{
		...
		ThreadingImpl.runTestThread(() -> {
			...
			for(ClientGameTestEntrypoint gameTest : gameTests)
			{
	...
	private static List<ClientGameTestEntrypoint> getTestToRun()
	{
		List<ClientGameTestEntrypoint> gameTests =
			ClientGameTestEntrypoint.getEntrypointContainers(ENTRYPOINT_KEY);
		String filter =
			System.getProperty(TestSystemProperties.MOD_ID_FILTER_PROPERTY);
		...
		List<String> modIds = Arrays.stream(filter.split(",")).map(String::trim)
			.filter(modId -> !modId.isEmpty()).peek(modId -> {
				if(!ModList.get().isLoaded(modId))
				{
					throw new IllegalArgumentException(
						"Mod ID %s specified in game test filter '%s' is not loaded"
							.formatted(modId, filter));
				}
			}).toList();
		...
		{
			throw new IllegalArgumentException(
				"No valid mod IDs specified in the client game test filter");
		}
		
		List<ClientGameTestEntrypoint> filteredGameTests = new ArrayList<>();
		
		for(ClientGameTestEntrypoint gameTest : gameTests)
		{
			if(modIds.contains(gameTest.getModId()))
			{
		...
		{
			throw new IllegalArgumentException(
				"No tests found for the specified mod IDs: " + modIds);
		}
